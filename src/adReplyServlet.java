...
@WebServlet(name = "adReplyServlet")
public class adReplyServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if(info.get(7).equals("user")){
            ...
            try{
                SalaryInfo=func.getreplyByAdmin(department);
            }catch (SQLException e) {
                e.printStackTrace();
            }
            application.setAttribute("salaryinfo",SalaryInfo);
            System.out.println(SalaryInfo);
            request.getRequestDispatcher("/replychart.jsp").forward(request,response);
        }
    }
